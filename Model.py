...
class GRU_With_Dropout(nn.Module):
    def __init__(self, input_size, hidden_size, dropout, bias=True):
        ...
        self.fc_net = FC_net(input_size=hidden_size*2,output_size=1,hidden_size=hidden_size,dropout=dropout,activ_out=None,nlayers=2,activation='Tanh')
    ...
    def forward(self, inputs, all_last):
        ...
        for t in range(inputs.size(1)):
            ...
            e = self.fc_net(torch.cat([tmp_h],dim=1)) # Pass the concatenated tensor through the fully connected network for attention
...
class DynamicDeepHitTorch(nn.Module):
    ...
    def __init__(self, input_dim, output_dim,network_settings, risks,optimizer='Adam'):
        ...
        self.Q_mha=nn.Linear(in_features=(input_dim-1)*2,out_features=(input_dim-1))
        init.xavier_uniform_(self.Q_mha.weight) # Initialize the weights of this layer
        self.V_mha = nn.Linear(in_features= self.h_dim1*2, out_features=self.h_dim1*2)
        init.xavier_uniform_(self.V_mha.weight) # Initialize the weights of this layer

        self.mha = nn.MultiheadAttention(embed_dim=self.h_dim1*2, num_heads=10,batch_first=True,dropout=self.keep_prob)
        self.query_linear = nn.Linear(30, 200) # For transforming last dimension of query.
        init.xavier_uniform_(self.query_linear.weight) # Initialize the weights of this layer
    ...
    def forward(self, x, x_mi):  # x represent the data and x_mi the mask data
        ...
        hidden = hidden.mul(rnn_mask_att.unsqueeze(-1).repeat(1, 1, 200))
        all_last = self.query_linear(all_last)
        all_last = all_last.unsqueeze(0).repeat(hidden.size(1), 1, 1).transpose(0,1)
        attn_output, attn_output_weights = self.mha(query=all_last, key=hidden, value=hidden)

        # Compute context vector
        c = torch.sum(hidden.mul(attn_output),dim=1)


        # Concatenate outputs from different risk networks
        outputs = []
        SB_out__ = torch.cat([x_last, c], dim=1)
        SB_out__ = (SB_out__ - SB_out__.min()) / (SB_out__.max() - SB_out__.min())
